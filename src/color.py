...
def display_obd_data_with_color(connection):
    ...
    class MockConnection:
        ...
        def query(self, cmd):
            # Simulate responses
            if cmd.name == "RPM":
                return MockResponse(1500 + time.time() % 500)
            if cmd.name == "SPEED":
                return MockResponse(30 + time.time() % 40)
            if cmd.name == "COOLANT_TEMP":
                return MockResponse(180 + (time.time() % 30))
        ...
        while True:
            ...
            for cmd in commands_to_monitor:
                ...
                if cmd.name == "COOLANT_TEMP" and isinstance(value, (int, float)):
                    ...
                    data_parts.append(
                        f"{COLOR_CYAN}{cmd.name}:{color_code}{value}°F{COLOR_RESET}"
                    )
                else:
                    data_parts.append(
                        f"{COLOR_CYAN}{cmd.name}:{COLOR_GREEN}{value}{COLOR_RESET}"
                    )

            # Join parts and print on the same line, overwriting previous
            output_line = " | ".join(data_parts)
            print(
                f"\r{output_line.ljust(80)}", end=""
            )  # ljust to clear previous longer lines
            sys.stdout.flush()
