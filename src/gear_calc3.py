def estimate_gear3(rpm, speed):
    """
    Estimate current gear based on RPM and speed.
    
    Args:
        rpm: Current engine RPM
        speed: Current speed in MPH
    
    Returns:
        str: Estimated gear ("1st", "2nd", "3rd", "4th (OD)", "N/P", or "---")
    """

    # Handle edge cases
    if speed < 1:
        return "N/P"  # Neutral/Park or too slow to determine

    if rpm == 0:
        return "---"  # No RPM data

    # Calculate current ratio (RPM per MPH)
    current_ratio = rpm / speed
    if current_ratio != 0:
        rounded_ratio = round(current_ratio, 2)
    else:
        rounded_ratio = 0

    class Gear:
        def __init__(self, name, min, max):
            self.name = name
            self.min = min
            self.max = max

# For now this will be all the ratios that are too high
# All of these gears should shift too late

# 140 is too early for first to second

# commenting this out for now, making sure it's greater than gemini estimate
    # GEARS = [
    #     Gear("1st", 140, 9999),
    #     Gear("2nd", 80, 139.99),
    #     Gear("3rd", 50, 79.99),
    #     Gear("4th (OD)", 0, 49.99),
    # ]

    GEARS = [
        Gear("1st", 120, 9999),
        Gear("2nd", 70, 119.99),
        Gear("3rd", 45, 69.99),
        Gear("4th (OD)", 0, 44.99),
    ]

    # get rid of "OD" in the 4th name, stupid

    for gear in GEARS:
        if gear.min <= rounded_ratio <= gear.max:
            return gear.name
    return "---"