...
from gear_calc3 import estimate_gear3
from gear_calc4 import estimate_gear4
from idle_calc import idle_ready
from stereo_screen import print_screen

BAR_WIDTH = 54
# Global state variable to hold gear across frames
last_gear = "N/P"
...
def gear_ratio_stats(gear, rpm, speed):
    ...
    return round(_gear_stats["min_ratio"], 2), round(
        _gear_stats["max_ratio"], 2
    )
...
def render_terminal(data_store):
    ...
    global last_gear  # Fixed: Declare global so state updates across loop calls
    ...
    # Fixed: Removed trailing comma that was forcing test_throttle to be a tuple
    test_throttle = data_store.get("Throttle Position", 0)
    ...
    if test_rpm != 0 and test_speed != 0:
        ...
        # Estimate gear with state retention
        test_gear2 = estimate_gear4(
            test_speed, test_rpm, test_throttle, last_gear
        )
        last_gear = test_gear2
    else:
        rounded_ratio = 0
        test_gear = "---"
        test_gear2 = "---"
    ...
    gear_header3, gear_fill3 = gear_indicator(test_gear2, BAR_WIDTH)
    ...
    lines.append(gear_header3)
    lines.append(gear_fill3)
    lines.append(divider)
